#!/usr/bin/env python3
"""
Automated Bundler for CIS Benchmarks Suite.
Concatenates Python benchmark audit logic into unified `audit_cis.py` CLI script.
...
def generate_unified_audit_script():
    """Generates audit_cis.py by embedding target metadata and execution logic."""
    target_file = os.path.join(REPO_ROOT, "audit_cis.py")
    print(f"📦 Bundling unified CIS audit script: {target_file} (v{VERSION})...")

    content = f'''#!/usr/bin/env python3
"""
Unified CIS Benchmark Audit Engine v{VERSION}
...
'''
    ...
    print(f"✅ audit_cis.py successfully bundled/updated (v{VERSION}, {len(content)} bytes)")
