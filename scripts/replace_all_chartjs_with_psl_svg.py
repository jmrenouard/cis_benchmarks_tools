...
for fpath in audit_files:
    ...
    # Replace category canvas template block
    content = re.sub(
        r'CATEGORY_CHART_CANVAS_TEMPLATE = """.*?"""',
        r'CATEGORY_CHART_CANVAS_TEMPLATE = "{svg_category_chart_html}"',
        content,
        flags=re.DOTALL
    )

    # Remove Chart.js <script> initialization blocks
    content = re.sub(r'<script>\s*// Données pour le graphique global.*?</script>', '', content, flags=re.DOTALL)

    with open(fpath, "w", encoding="utf-8") as f:
        f.write(content)

print("✅ Replaced Chart.js with 100% Pure Inline SVG Charts across all audit scripts!")
