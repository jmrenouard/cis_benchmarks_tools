#!/usr/bin/env python3
"""
1. Link all 18 audit scripts to load external audit rules from `rules/<target_key>.json`.
2. Inject 100% Pure Inline SVG Donut & Stacked Bar Charts (PSL ONLY, zero JS, 100% offline).
"""
...
print(f"Linking external rules and inline SVG charts across {len(audit_files)} audit scripts...")
...
for fpath in audit_files:
    ...
    # Inject rule_loader_code and psl_chart_engine
    if "def load_recommendations(" not in content:
        content = content.replace("def run_command(", rule_loader_code + "\n\n" + psl_chart_engine + "\n\n\ndef run_command(")

    # In perform_checks or __main__, load recommendations via load_recommendations
    content = content.replace(
        f'check_results = perform_checks(RECOMMENDATIONS_DATA, remote_host=remote_target)',
        f'rules_data = load_recommendations("{target_key}")\n    check_results = perform_checks(rules_data, remote_host=remote_target)'
    )

    with open(fpath, "w", encoding="utf-8") as f:
        f.write(content)

print("✅ Successfully linked external rules from `rules/` and injected inline SVG charts!")
