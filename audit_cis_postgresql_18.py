...
RECOMMENDATIONS_DATA = [
    ...
    {"category": "3. Journalisation et audit", "number": "3.2", "name": "Activer l’extension pgAudit", "type": "Automated", "test_procedure": "sudo -u postgres psql -t -c \"SELECT * FROM pg_extension WHERE extname = 'pgaudit';\"", "expected_output": {"type": "stdout_contains", "value": "pgaudit"}, "remediation": "Installer et configurer l’extension d’audit avancé pgAudit pour capturer les activités.\nCREATE EXTENSION pgaudit; ALTER SYSTEM SET pgaudit.log = 'all'; SELECT pg_reload_conf();"}, # Check if query returns 1 (extension exists), added description, changed expected output
    ...
    {"category": "4. Accès et autorisations utilisateur", "number": "4.8", "name": "Installer l’extension set_user", "type": "Automated", "test_procedure": "sudo -u postgres psql -t -c \"SELECT * FROM pg_extension WHERE extname = 'set_user';\"", "expected_output": {"type": "stdout_contains", "value": "set_user"}, "remediation": "Utiliser set_user pour l’émulation de rôles et la révocabilité de sessions.\nCREATE EXTENSION set_user;"}, # Added description, changed expected output to check for extension name in output
...
<head>
    ...
    <title>CIS Benchmark Audit Report PostgreSQL 18 Benchmark</title>
...
<body class="font-sans bg-gray-100 text-gray-800 p-6">
    <div class="container mx-auto bg-white p-8 rounded-lg shadow-lg">
        <h1 class="text-3xl font-bold mb-6 text-gray-900">CIS Benchmark Audit Report PostgreSQL 18 Benchmark</h1>
...
def generate_html_report(results, overall_score, categories_scores, filename=None, lang="en", execution_context=None):
    if not filename:
        filename = "reports/rapport_cis_postgresql_18.html"
    ...
    html_output = load_html_template().format_map(SafeDict(
        product_title="PostgreSQL 18",
        benchmark_title="PostgreSQL 18",
...
if __name__ == "__main__":
    ...
    rules_data = load_recommendations("postgresql_18")
    ...
    export_results(check_results, overall_score, categories_scores, target_name="postgresql_18", filename=args.output, fmt=args.format, lang=args.lang)