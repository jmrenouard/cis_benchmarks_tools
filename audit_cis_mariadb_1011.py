...
MYSQL_CMD = "mariadb -N -B || mysql -N -B" # -N: skip headers, -B: batch mode (tab separated)
...
RECOMMENDATIONS_DATA = [
    ...
    {"category": "7. Authentification", "number": "7.2", "name": "Aucun mot de passe dans la configuration globale", "type": "Automated", "test_procedure": "! grep -riE 'password|pwd|pass' /etc/mysql/ /etc/my.cnf | grep -v '#' | grep -v 'password_lifetime' | grep -v 'password_check' | grep -v 'validate_password' | grep -v 'strict_password' | grep -v 'old_passwords' | grep -v 'default_password_lifetime' | grep -v 'password_last_changed'", "expected_output": {"type": "returncode_zero"}, "remediation": "Utiliser des fichiers .my.cnf privés ou mysql_config_editor avec permissions restreintes."},
...
# --- Modèle HTML (Adapté pour MariaDB 10.11) ---
HTML_TEMPLATE = """
<!DOCTYPE html>
...
<head>
    ...
    <title>CIS Benchmark Audit Report MariaDB 10.11 Benchmark</title>
...
<body class="bg-gray-50 flex">
    ...
    <aside class="w-64 h-screen bg-white border-r border-gray-200 sticky top-0 overflow-y-auto hidden lg:block">
        <div class="p-6">
            <h2 class="text-xl font-bold text-blue-600"><i class="fas fa-shield-halved mr-2"></i>CIS MariaDB 10.11</h2>
    ...
    <main class="flex-1 min-w-0">
        ...
        <header class="bg-white border-b border-gray-200 p-6 flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Benchmark CIS MariaDB 10.11</h1>
                <p class="text-sm text-gray-500">Report Date: {report_date}</p>
            </div>
            <div class="flex space-x-2">
                <span class="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">MariaDB 10.11.x</span>
                <span class="px-3 py-1 bg-gray-100 text-gray-800 rounded-full text-xs font-medium border border-gray-200">v1.0.0</span>
            </div>
        </header>
        ...
        <footer class="bg-white border-t border-gray-200 p-8 mt-12 text-center">
            <p class="text-sm text-gray-500 italic">Rapport généré automatiquement par CIS MariaDB 10.11 Auditor.</p>
            <p class="text-xs text-gray-400 mt-2">Basé sur CIS MariaDB 10.11 Benchmark v1.0.0.</p>
        </footer>
    </main>
...
def generate_html_report(results, overall_score, categories_scores, filename=None, lang="en", execution_context=None):
    if not filename:
        filename = "reports/rapport_cis_mariadb_1011.html"
