...
<head>
    ...
    <title>Rapport CIS MariaDB 10.11 Benchmark</title>
...
def run_command(command):
    ...
    try:
        ...
        return "", f"Erreur : Commande '{cmd_name}' introuvable.", 127
                ...
                if cmd_to_run:
                    ...
                    if rec["type"] == "Manual":
                        ...
                        check_result["output"] = f"Erreur: Timeout.\n{check_result['output']}"
                         ...
                         check_result["status"] = "Error"
                         check_result["output"] = f"Erreur: Accès refusé. Vérifiez les identifiants/privilèges MariaDB.\n{check_result['output']}"
                    elif "ERROR 2002 (HY000): Can't connect" in stderr:
                         check_result["status"] = "Error"
                         check_result["output"] = f"Erreur: Impossible de se connecter à MariaDB (serveur arrêté ou mauvais socket).\n{check_result['output']}"
                    elif condition:
...
if __name__ == "__main__":
    print("🚀 Démarrage de l'audit CIS MariaDB 10.11 Benchmark v1.0.0 ...")
    print(f"ℹ️ Utilisation de la commande MySQL/MariaDB: '{MYSQL_CMD}' (Assurez-vous que la connexion est configurée)")
    ...
    try:
        ...
        print("Consulte le fichier rapport_cis_mariadb_1011.html pour les détails.")
