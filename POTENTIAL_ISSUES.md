...
### 2. Phase 20 History Check Command Exit Code & Empty Path_Command Handling (Resolved in v2.6.4 ✅)
...
- [x] **PR #344**: `fix(rules): sanitize test_procedure commands and rule 2.1.1 in MySQL 8.0 specification` — *Eliminated corrupted dummy manual echo strings in rules/mysql_80.json and hardened rule 2.1.1 fallback when crontab binary is missing (Part 1 of #343).*
...
- [x] **PR #396**: `feat(docker): daemon probe, container discovery, and transport resolver (Part 1a)` — *Implemented DockerDaemonProbe, ContainerInfo, DockerContainerDiscovery, and DockerTransportResolver in docker_transport.py (Issue #395).*
- [x] **PR #397**: `test(docker): unit tests for transport discovery and resolver (Part 1b)` — *Added unit tests in tests/test_docker_transport.py (Issue #395).*
- [x] **PR #398**: `feat(execution): in-container DockerContainerExecutor and product discovery (Part 2)` — *Enhanced DockerExecutor with get_container_info(), is_container_running(), and auto-discovery in create_executor (Issue #395).*
- [x] **PR #399**: `feat(container): in-container POSIX and filesystem inspection engine (Part 3)` — *Implemented InContainerInspector and PosixStatResult in in_container_inspector.py (Issue #395).*
- [x] **PR #400**: `feat(e2e): docker in-container E2E execution and dashboard engine (Part 4)` — *Implemented DockerE2EOrchestrator and TargetE2ESpec in docker_e2e_engine.py (Issue #395).*
- [x] **PR #401**: `feat(orchestration): runtime container metadata injection and transport resolution (Part 5)` — *Integrated DockerTransportResolver in AuditOrchestrator and enriched TargetAuditExecutionResult (Issue #395).*
- [x] **PR #402**: `test(isolation): adversarial in-container isolation and fault resilience suite (Part 6a)` — *Added tests/test_in_container_isolation.py validating zero host leakage, non-root UID/GID, and read-only rootfs (Issue #395).*
- [x] **PR #403**: `docs(release): version bump v2.7.0, roadmap, and potential issues sync (Part 6b)` — *Released v2.7.0, synchronized Phase 26 in ROADMAP.md and POTENTIAL_ISSUES.md (closes #395).*
- [x] **PR #405**: `feat(temporal): real-time timestamping and timezone metadata module (Part 1)` — *Implemented TemporalAuditMetadata and get_local_timezone_info in temporal_metadata.py (Issue #404).*
- [x] **PR #406**: `feat(pipeline): crash-resilient post-execution report publisher with atomic writing (Part 2)` — *Implemented AuditReportPublisher and atomic_write_text in post_execution_publisher.py (Issue #404).*
- [x] **PR #407**: `feat(template): dynamic temporal header and clean thematic security rendering (Part 3)` — *Refactored templates/report_template.html to eliminate hardcoded mock bars and render real-time headers (Issue #404).*
- [x] **PR #408**: `feat(mariadb106): temporal metadata and post-execution publishing integration (Part 4a)` — *Integrated TemporalAuditMetadata, dynamic thematics, and AuditReportPublisher in audit_cis_mariadb_106.py (Issue #404).*
- [x] **PR #409**: `test(mariadb106): unit tests for report publishing and temporal metadata (Part 4b)` — *Added tests/test_mariadb_106_report_publishing.py (Issue #404).*
- [x] **PR #410**: `feat(orchestration): temporal metadata propagation in orchestrator and E2E engine (Part 5)` — *Propagated TemporalAuditMetadata across audit_orchestrator.py and docker_e2e_engine.py (Issue #404).*
- [x] **PR #411**: `test(temporal): adversarial real-time timestamping and I/O integrity test harness (Part 6a)` — *Added tests/test_realtime_timestamp_adversarial.py validating sub-2s clock delta, disk write presence, and overwrite integrity (Issue #404).*
- [x] **PR #412**: `docs(release): version bump v2.8.0, roadmap, and potential issues sync (Part 6b)` — *Released v2.8.0, synchronized Phase 27 in ROADMAP.md and POTENTIAL_ISSUES.md (closes #404).*
