# ⚠️ Potential Issues & Technical Debt Backlog (v1.4.0)

Ce document recense les problèmes potentiels, risques de bugs, remarques de sécurité et retours des revues de code (Pull Requests #17 à #87) identifiés sur le projet **CIS Benchmarks Tools**.
...
## 1. Résolus dans les versions v1.2.0 - v1.4.0 ✅
...
- [x] **Élimination de `shell=True` & Sécurisation `subprocess`** : Migration de 100% des exécutions `subprocess` vers le format liste de paramètres stricts (`['/bin/bash', '-c', command]` et tableaux `['docker', ...]`). Risque d'injection de commande éradiqué.
