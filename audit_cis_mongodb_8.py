...
def generate_html_report(results, overall_score, categories_scores, filename=None, lang="en", execution_context=None):
    if not filename:
        filename = "reports/rapport_cis_mongodb_8.html"
    ...
    html_output = load_html_template().format_map(SafeDict(
        product_title="MongoDB 8",
        benchmark_title="MongoDB 8",
...
if __name__ == "__main__":
    ...
    rules_data = load_recommendations("mongodb_8")
    ...
    export_results(check_results, overall_score, categories_scores, target_name="mongodb_8", filename=args.output, fmt=args.format, lang=args.lang)