...
<head>
    ...
    <title>CIS Benchmark Audit Report MySQL 8.0 Benchmark</title>
...
<body class="bg-gray-50 flex">
    ...
    <main class="flex-1 min-w-0">
        ...
        <header class="bg-white border-b border-gray-200 p-6 flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Benchmark CIS MySQL 8.0</h1>
                <p class="text-sm text-gray-500">Report Date: {report_date}</p>
            </div>
            <div class="flex space-x-2">
                <span class="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">MySQL 8.0.x</span>
        ...
        <footer class="bg-white border-t border-gray-200 p-8 mt-12 text-center">
            ...
            <p class="text-xs text-gray-400 mt-2">Basé sur CIS MySQL 8.0 Benchmark v1.0.</p>
        </footer>
    </main>
            ...
            try:
                ...
                if rec.get("path_command"):
                    ...
                    if (path_returncode != 0 or not path_stdout) and "datadir" in path_cmd:
                        fb_stdout, fb_stderr, fb_ret = run_command("ls -d /var/lib/mariadb /var/lib/mysql 2>/dev/null | head -n 1", remote_host=remote_host, docker_container=docker_container)
...
def generate_html_report(results, overall_score, categories_scores, filename=None, lang="en", execution_context=None):
    if not filename:
        filename = "reports/rapport_cis_mysql_80.html"
    ...
    html_output = load_html_template().format_map(SafeDict(
        product_title="MySQL 8.0",
        benchmark_title="MySQL 8.0",
